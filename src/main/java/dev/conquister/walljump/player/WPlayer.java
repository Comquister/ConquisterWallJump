package dev.conquister.walljump.player;

import com.tcoded.folialib.wrapper.task.WrappedTask;
import dev.conquister.walljump.WallJump;
import dev.conquister.walljump.api.events.WallJumpEndEvent;
import dev.conquister.walljump.api.events.WallJumpResetEvent;
import dev.conquister.walljump.api.events.WallJumpStartEvent;
import dev.conquister.walljump.config.WallJumpConfiguration;
import dev.conquister.walljump.enums.WallFace;
import dev.conquister.walljump.handlers.WorldGuardHandler;
import dev.conquister.walljump.utils.BukkitUtils;
import dev.conquister.walljump.utils.EffectUtils;
import dev.conquister.walljump.utils.LocationUtils;
import dev.conquister.walljump.utils.VelocityUtils;
...
public class WPlayer {
    ...
    private WrappedTask velocityTask;
    private WrappedTask fallTask;
    private float velocityY;
    private WrappedTask stopWallJumpingTask;
    ...
    public void onWallJumpStart() {
        try {
            ...
            // play sound and spawn particles is bugged.
            ...
            // Use FoliaLib instead of Bukkit scheduler
            velocityTask = WallJump.getInstance().getFoliaLib().getScheduler().runAtEntityTimer(player, () -> {
                ...
                if (velocityY != 0) {
                    ...
                    if (sliding) {
                        if (player.isOnGround() || !Objects.requireNonNull(LocationUtils.getBlockPlayerIsStuckOn(player, lastFacing)).getType().isSolid()) {
                            ...
                            WallJump.getInstance().getFoliaLib().getScheduler().runAtEntity(player, task -> {
                        ...
                        if (lastJumpLocation.getY() - player.getLocation().getY() >= 1.2) {
                            lastJumpLocation = player.getLocation();
                            WallJump.getInstance().getFoliaLib().getScheduler().runAtEntity(player, task -> EffectUtils.playWallJumpSound(player, lastFacing, 0.2f, 0.6f));
                        }
                    }
                }
            }, 0, 1);
            ...
            fallTask = WallJump.getInstance().getFoliaLib().getScheduler().runAtEntityLater(player, () -> {
                if (onWall) {
                    if (config.getBoolean("slide")) {
                        ...
                        WallJump.getInstance().getFoliaLib().getScheduler().runAtEntity(player, task -> this.onWallJumpEnd());
                    }
                }
            ...
            WallJump.warning("Failed to start wall jump for player " + player.getName() + "!");
        }
    }
    ...
    public void onWallJumpEnd(boolean jump) {
        try {
            ...
            // Check if velocityTask is not null before canceling
            ...
            WallJump.getInstance().getFoliaLib().getScheduler().runAtEntityLater(player, () -> {
            ...
            stopWallJumpingTask = WallJump.getInstance().getFoliaLib().getScheduler().runAtEntityLater(player, this::reset, 24);
        }catch (IllegalArgumentException | IllegalStateException e){
            WallJump.warning("Failed to end wall jump for player " + player.getName() + "!");
        }
    }
    ...
    private void reset() {
        try {
            ...
            WallJump.warning("Failed to reset wall jump for player " + player.getName() + "!");
        }
    }
    ...
    public boolean canWallJump() {
        try {
            ...
            WallJump.warning("Failed to check if player " + player.getName() + " can wall jump!");
            return false;
        }
    }
